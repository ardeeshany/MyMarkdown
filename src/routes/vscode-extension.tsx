...
const FEATURES = [
  { icon: Palette, title: "Styled preview", body: "VS Code's own preview (Cmd/Ctrl + Shift + V) gets the MyMarkdown look: vivid H1–H3 headings, JSON one field per line with coloured field names, task lists, Mermaid diagrams drawn in your theme's colours, and calm code blocks — with scroll sync and click-to-source built in." },
    ...
    <main className="relative min-h-screen overflow-x-clip bg-background px-5 pb-20 pt-6 text-foreground sm:px-8">
      ...
      <div className="relative mx-auto max-w-3xl">
        ...
        <header className="mt-10 flex flex-col items-center text-center">
          ...
          <div className="mt-6">
            <Button asChild size="lg" className="h-12 gap-2.5 rounded-full bg-primary px-6 text-sm font-medium text-primary-foreground shadow-lg shadow-primary/25 hover:bg-primary/90">
              <a href={VSIX} download target="_blank" rel="noopener noreferrer">
                <Download />Download for VS Code
              </a>
            </Button>
          </div>
          <p className="mt-2.5 text-xs text-muted-foreground">Version {VERSION} · about 1 MB · works with VS Code 1.85+</p>
        </header>
