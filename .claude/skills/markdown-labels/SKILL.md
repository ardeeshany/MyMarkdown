...
Leave `sourceHash` out. The extension recomputes it, and a wrong hash is worse than a
missing one — it would claim the ranges match text they were never checked against.

Do not add `anchor` or `endAnchor` either. The extension fills those in from the document
itself, which is the only way they can be right.

If the file already exists, keep any lenses already in it whose names you are not
replacing.

## After writing

Tell the person in one line that you labelled the document and what the three lenses are.
They switch between them from the MyMarkdown item in the VS Code status bar.
